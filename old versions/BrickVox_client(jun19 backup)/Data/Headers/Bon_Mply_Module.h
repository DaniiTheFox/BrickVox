 ...
 while(std::getline(m_tmp, tmp_line)){                                        // mientras no termine de leer el archivo
  ...
  //std::cout<<"declaring temporals\n";
   ...
   for(int i = 0; i < strlen(tmp.c_str()); i++){
    ...
    if(ns1 == true){                                                          // proceso necesario para la conversion a un formato legible
    if(tmp[i] != '/'){
     if(isL == true){
     num1 = num1 + tmp[i];
     }else{
     num2 = num2 + tmp[i];                                                    // el obj tiene datos asi: 45/6 23/55 12/43
     }
    }else{                                                                    // el proposito es descomponer esos datos
    isL = false;
    }
    }
    if(tmp2[i] == '.'){                                                       // para obtenerlos de forma individual cada uno y convertirlos a
     ns2 = false;                                                             // variables enteras
    }
    if(ns2 == true){
    if(tmp2[i] != '/'){
     if(isL2 == true){
     num3 = num3 + tmp2[i];                                                   // por eso se repite el proceso de investigar cual es el caracter
     }else{
     num4 = num4 + tmp2[i];
     }                                                                        // y depende el resultado genera string
    }else{
    isL2 = false;
    }
    }
    if(tmp3[i] == '.'){
     ns3 = false;                                                             // al final cada dato queda separado de forma individual
    }
    if(ns3 == true){
    if(tmp3[i] != '/'){
     if(isL3 == true){
     ...
     num6 = num6 + tmp3[i];
     }                                                                      //el proceso se repite por cada dato en las 3 partes del f en el obj
    }else{
    isL3 = false;
    }
    }
